...
############# GA enhanced
# (E1) Tournament selection (k=tourn_k) instead of random parent choice.
# (E2) Order Crossover (OX) to preserve valid TSP permutations.
# (E3) Multi-elitism: keep top 'elite' individuals each generation.
# (E4) Time control: stop early if time.time() - start_time > time_limit.
#
pop_size = 100
max_it = 1000
time_limit = 55
...
def tournament_select(population, k):
    ...
    best = candidates[0]
    bestL = compute_length(best)
    for ind in candidates[1:]:
        L = compute_length(ind)
        if L < bestL:
            best = ind
            bestL = L
    return best[:]
...
best_length = compute_length(best_tour)
...
for _gen in range(max_it):
    ...
    for ind in population:
        scored.append((compute_length(ind), ind))

    scored.sort(key=lambda x: x[0])

    for e in range(elite):
        new_pop.append(scored[e][1][:])

    while len(new_pop) < pop_size:
        p1 = tournament_select(population, tourn_k)
        p2 = tournament_select(population, tourn_k)
