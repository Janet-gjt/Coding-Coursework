...
############# AC enhanced
# (E1) Elite reinforcement:
#      After standard pheromone deposition, add extra pheromone on the
#      globally best tour found so far to increase exploitation.
#      tau[a][b] += elite_weight * Q / best_length
#
# (E2) Robust heuristic matrix:
#      Use eta = 1/d but guard against d==0 to avoid division-by-zero.
#
# (E3) Time control:
#      Stop early if time.time() - start_time > time_limit.
#
max_it = 200
num_ants = 20
alpha = 1
...
tau = [[1.0 for _ in range(num_cities)] for _ in range(num_cities)]

eta = [[0 for _ in range(num_cities)] for _ in range(num_cities)]
...
def compute_length(t):
    ...
    for i in range(num_cities - 1):
        length += dist_matrix[t[i]][t[i+1]]
    length += dist_matrix[t[-1]][t[0]]
    return length
...
for iteration in range(max_it):
    ...
    for ant in range(num_ants):
        ...
        while unvisited:
            ...
            for city in unvisited:
                prob = (tau[current_city][city] ** alpha) * (eta[current_city][city] ** beta)
    ...
    for k in range(len(all_tours)):
        ...
        tau[tour_k[-1]][tour_k[0]] += Q / length_k
        tau[tour_k[0]][tour_k[-1]] += Q / length_k
    ...
    for i in range(num_cities - 1):
        ...
        b = best_tour[i+1]
        tau[a][b] += elite_weight * Q / best_length
        tau[b][a] += elite_weight * Q / best_length

    tau[best_tour[-1]][best_tour[0]] += elite_weight * Q / best_length
    tau[best_tour[0]][best_tour[-1]] += elite_weight * Q / best_length


tour = best_tour
tour_length = int(best_length)
